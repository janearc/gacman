...
type Chunk struct {
	Position Vector3         // The world position of this chunk (e.g., its center or corner)
	Size     int             // The size of the chunk (e.g., number of cells along one axis)
	Cells    map[string]Cell // A map of cells within this chunk, keyed by their local coordinates (e.g., "0,0,0")
}

// NewChunk creates a new Chunk with the specified position and size.
func NewChunk(position Vector3, size int) Chunk {
	return Chunk{
		Position: position,
		Size:     size,
...
func (c *Chunk) AddCell(localCoord string, cell Cell) {
	c.Cells[localCoord] = cell
}

// GetCell retrieves a Cell from the chunk by its local coordinates.
func (c *Chunk) GetCell(localCoord string) (Cell, bool) {
